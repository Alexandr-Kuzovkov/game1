@bin\spatialite_osm_net -o osm\%1 -d sqlite\%1.sqlite --roads -T roads -tf road_template.conf
@ping -n 1 -w 2000 192.168.254.254 > nul
@bin\spatialite_network -d sqlite\%1.sqlite -T roads -f node_from -t node_to -g geometry --oneway-tofrom oneway_tofrom --oneway-fromto oneway_fromto -n name -o roads_net_data --overwrite-output
@ping -n 1 -w 2000 192.168.254.254 > nul
@bin\spatialite sqlite\%1.sqlite "CREATE VIRTUAL TABLE roads_net USING VirtualNetwork('roads_net_data')"
